...
# Plot
fig, ax = plt.subplots(len(freqs), figsize=(6, 2), sharex=True, sharey=True, squeeze=False)
ax = ax[0]
fig.subplots_adjust(top=0.95, bottom=0.21, left=0.1)
...
for i in range(len(freqs)):
    ...
    #                (1.05, 0.5), xycoords='axes fraction')
ax[0].annotate(r'$80\pm 15$ rpm', (85, 6), fontsize=10)
ax[0].annotate(r'$105\pm 6$ rpm', (103, 19), fontsize=10)
ax[0].annotate(r'$125\pm 10$ rpm', (125, 30), fontsize=10)
# ax[0].annotate(('Response to harmonic generator\nspeed variations at ' +
#                 r'$\omega = {:.2f}$ rad/s'.format(freqs[0])),
#                (119, 41), (100, 41), ha='right', va='center',
#                arrowprops=dict(arrowstyle='->'))
# ax[0].annotate('Harmonic', (115.5, 35), (100, 35), ha='right',
#                va='center', color='red',
#                arrowprops=dict(arrowstyle='->', color='red', alpha=0.6))
ax[0].annotate('Optimum torque curve',
               (97, 21), (95, 31), ha='center', va='bottom', color='blue',
               arrowprops=dict(arrowstyle='->', color='blue', alpha=0.4))
ax[0].annotate('Constant power',
               (105, 50), (98, 45), ha='center', va='top', color='blue',
               arrowprops=dict(arrowstyle='->', color='blue', alpha=0.4))

ax[0].axvline(controller.params['cut in speed'], ls=':', color='grey', lw=0.5)
ax[0].axvline(controller.params['rated speed'], ls=':', color='grey', lw=0.5)
ax[0].annotate('Cut-in speed', (71, 48), fontsize=10, color='grey')
ax[0].annotate('Rated speed', (122.5, 48), fontsize=10, color='grey')

ax[0].set_ylabel('Generator torque [kNm]')
ax[0].set_xlabel('Generator speed [rpm]')
